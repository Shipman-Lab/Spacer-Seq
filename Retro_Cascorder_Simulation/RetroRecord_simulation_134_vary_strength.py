...
rate_A_on=(6.46E-5)*16, rate_A_off=(1.21E-5)*16, rate_B_on=6.47E-5, rate_B_off=4.37E-5,
...
for i in range(-4,5):
	data_constant_A = []
	for j in range(-4,5):
		data_constant_A.append([run_single_experiment(population_size = (1000000),
		rate_A_on=(6.46E-5)*(2**i), rate_A_off=(1.21E-5)*(2**i),
		rate_B_on=(6.47E-5)*(2**j), rate_B_off=(4.37E-5)*(2**j),
		order_of_expression='A_before_B')['composite_score'] for k in range(50)])
	data_AB.append(data_constant_A)

print(data_AB)
workbook_134_AB = xlsxwriter.Workbook("134_AB_sim_vary_strength_comp2.xlsx")
sheets = []
for i in range(len(data_AB)):
	sheets.append(workbook_134_AB.add_worksheet("R_A x 2e" + str(i-4)))
	row, col = 0, 0
	sheets[i].write(row, col,"Rate B")
	row += 1
	for j in range(len(data_AB[i])):
		sheets[i].write(row, col, "R_B x 2e" + str(j-4))
		col += 1
		for k in range(len(data_AB[i][j])):
			# sheets[i].write(row, col, str(i)+","+str(j)+","+str(k))
			sheets[i].write(row, col, data_AB[i][j][k])
			col += 1
		row += 1
		col = 0
