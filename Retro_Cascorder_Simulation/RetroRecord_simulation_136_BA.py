...
rate_A_on=4.01E-4, rate_A_off=3.77E-4, rate_B_on=7.13E-5, rate_B_off=4.12E-6,
rate_N=2.39E-3, time_of_expression_A=24, time_of_expression_B=24,
order_of_expression='B_before_A', subinterval_length = 0.1):
...
data_BA = [run_single_experiment(population_size=1000000) for i in range(100)]
...
values_BA = [np.array([elem['AN_score'] for elem in data_BA]),
np.array([elem['BN_score'] for elem in data_BA]),
np.array([elem['AB_score'] for elem in data_BA]),
np.array([elem['composite_score'] for elem in data_BA])]
...
workbook_136_BA = xlsxwriter.Workbook('136_BA_sim_comp2.xlsx')
sheet1_136_BA = workbook_136_BA.add_worksheet()
sheet1_136_BA.write(0, 0, 'A/N')
sheet1_136_BA.write(1, 0, 'B/N')
sheet1_136_BA.write(2, 0, 'A/B')
sheet1_136_BA.write(3, 0, 'composite')

row, col = 0, 1
for rule in values_BA:
    for score in rule:
        sheet1_136_BA.write(row, col, score)
        col += 1
    row += 1
    col = 1

workbook_136_BA.close()
